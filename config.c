...
void get_config()
{
	...
	while(!flag) {
		...
		cprint(POP_Y+9,  POP_X+6, "(7) CPU Selection Mode");
		...
		case 2:
			...
			while (!sflag) {
				...
				case 2:
					...
					find_ticks_for_pass();
					sflag++;
					cprint(LINE_INFO, COL_INF3-3, "Std");
					break;
				case 3:
					/* Skip test */
					bail++;
				...
				case 4:
					...
					sflag++;
                                        bail++;
					cprint(LINE_INFO, COL_INF3-4, "  #");
					dprint(LINE_INFO, COL_INF3-1, i, 1, 0);
					break;
				case 5:
					if (v->testsel != 9) {
						v->pass = -1;
		...
		case 8:
			...
			cprint(POP_Y+3, POP_X+6, "(1) Single");
