...
void mem_size(void)
{
	...
	case SZ_MODE_PROBE:
		...
		memsize_probe();
		cprint(LINE_INFO+1, COL_INF1-6, "Probed");
		break;
	}
	...
	adj_mem();
	aprint(LINE_INFO+2, COL_INF1-5, v->reserved_pages);
}
...
static void memsize_linuxbios(void)
{
	...
	v->msegs = n;
	cprint(LINE_INFO+1, COL_INF1-6, "LxBIOS");
}
static void memsize_820()
{
	...
	v->msegs = n;
	cprint(LINE_INFO+1, COL_INF1-6, "  e820");
}
...
static void memsize_801(void)
{
	...
	if (alt_mem_k < ext_mem_k) {
		mem_size = ext_mem_k;
		cprint(LINE_INFO+1, COL_INF1-6, "   e88");
	} else {
		mem_size = alt_mem_k;
		cprint(LINE_INFO+1, COL_INF1-6, "  e801");
	}
